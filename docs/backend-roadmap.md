...
把当前本地演示后端演进为可私有化部署的工程项目多智能体管理后端。路线采用模块化单体优先，先稳定业务边界、数据模型和任务链路，再考虑拆分服务。

## Phase 1: 稳定后端边界

- 保留当前原生 Node 演示服务，新增 repository 适配层，路由层不再直接依赖内存数据实现。
- 把业务域固定为：组织与项目、进度、资料、安全、技术成本、通知、Agent 编排。
...
- 引入 PostgreSQL，迁移核心实体：Organization、Project、ProjectMember、WBSNode、Milestone、Document、DocumentVersion、SafetyInspection、RiskItem、RectificationTask、CostSnapshot、TechScheme、AgentRun、AgentInsight、Notification。
...
- API 支持公司层总览、项目下钻、进度导入、资料上传、安全巡检、整改反馈、Agent 问答、异步 AgentRun、站内通知。
- repository 边界已建立，当前实现为内存仓储，后续可替换为 PostgreSQL 仓储。
- 项目级审计日志已建立，覆盖资料上传/索引、进度导入、安全巡检/整改反馈和 AgentRun 状态流转。
